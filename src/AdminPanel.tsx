...
export default function AdminPanel() {
  ...
  // Authorization check
  const isAuthorized = (user?.email === ADMIN_EMAIL) || isTgAdmin || (userData?.telegramId && (ADMIN_TG_IDS.includes(userData.telegramId) || (settings.adminIds && settings.adminIds.includes(userData.telegramId))));
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl space-y-8 text-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-500 text-sm">You do not have permission to access this panel.</p>
          </div>
