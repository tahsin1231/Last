...
━━━━━━━━━━━━━━━━━━━━━`,
  ...
  const handleAddAdmin = async () => {
    ...
    try {
      const userRef = doc(db, 'users', adminIdInput);
        ...
        {activeAdminTab === 'admins' && (
          <div className="space-y-6">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
              ...
              <p className="text-xs text-slate-500">Note: The user must have opened the app at least once to be in the database.</p>
            </div>

            <div className="space-y-4">
              <h3 className="text-lg font-bold px-2">Current Admins</h3>
              {allUsers.filter(u => u.isAdmin).map(u => (
                <div key={u.telegramId} className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <img src={u.photoUrl} className="w-12 h-12 rounded-xl object-cover border border-slate-800" referrerPolicy="no-referrer" />
                    <div>
                      <p className="font-bold">{u.firstName} {u.telegramId === ADMIN_TG_ID && <span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full ml-2">OWNER</span>}</p>
