...
export default function AdminPanel() {
  ...
  // Authorization check
  const isAuthorized = (user?.email === ADMIN_EMAIL) || isTgAdmin || isPasswordAuthenticated || (userData?.telegramId && (ADMIN_TG_IDS.includes(userData.telegramId) || settings.adminIds?.includes(userData.telegramId)));

  useEffect(() => {
    const checkAdmin = () => {
      const tgUser = window.Telegram?.WebApp?.initDataUnsafe?.user;
      if (tgUser && (ADMIN_TG_IDS.includes(tgUser.id.toString()) || settings.adminIds?.includes(tgUser.id.toString()))) {
        setIsTgAdmin(true);
      }
    };
            ...
            <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <button 
                    onClick={() => {
                      if (newAdminId && !settings.adminIds.includes(newAdminId)) {
