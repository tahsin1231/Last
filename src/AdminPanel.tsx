...
━━━━━━━━━━━━━━━━━━━━━`,
  ...
  const [activeAdminTab, setActiveAdminTab] = useState<'dashboard' | 'settings' | 'withdrawals' | 'users'>('dashboard');
  ...
  // Authorization check
  const isAuthorized = (user?.email === ADMIN_EMAIL) || isTgAdmin || isPasswordAuthenticated || (userData?.telegramId && ADMIN_TG_IDS.includes(userData.telegramId));

  useEffect(() => {
    const checkAdmin = () => {
      const tgUser = window.Telegram?.WebApp?.initDataUnsafe?.user;
      if (tgUser && ADMIN_TG_IDS.includes(tgUser.id.toString())) {
        setIsTgAdmin(true);
      }
    };
