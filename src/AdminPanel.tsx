...
━━━━━━━━━━━━━━━━━━━━━`,
  ...
  const [pendingAdminChanges, setPendingAdminChanges] = useState<Record<string, boolean>>({});
  ...
  // Authorization check
  const isAuthorized = (user?.email === ADMIN_EMAIL) || isTgAdmin || isPasswordAuthenticated || userData?.isAdmin;
  ...
  const handleAddAdmin = async () => {
    ...
    const userToAdmin = allUsers.find(u => u.telegramId === adminIdInput);
    if (!userToAdmin) {
      alert('User not found in database. They must open the app first.');
      return;
    }
    setPendingAdminChanges(prev => ({ ...prev, [adminIdInput]: true }));
    setAdminIdInput('');
  };

  const handleRemoveAdmin = (telegramId: string) => {
    if (telegramId === ADMIN_TG_ID) {
      alert('Cannot remove the main owner admin.');
      return;
    }
    setPendingAdminChanges(prev => ({ ...prev, [telegramId]: false }));
  };

  const handleSaveAdmins = async () => {
    setSaving(true);
    try {
      for (const [telegramId, isAdmin] of Object.entries(pendingAdminChanges)) {
        ...
        if (userSnap.exists()) {
          ...
          if (isAdmin) {
            if (userData.firebaseUid) {
              await setDoc(doc(db, 'admins', userData.firebaseUid), { 
                telegramId,
            ...
            if (userData.firebaseUid) {
              await deleteDoc(doc(db, 'admins', userData.firebaseUid));
            }
          }
        }
      }
      ...
      console.error(err);
      alert('Error saving admin changes');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase">Earning Per Ad (USD)</label>
                  <input type="number" step="0.0001" value={settings.earningPerAd} onChange={e => setSettings({...settings, earningPerAd: parseFloat(e.target.value)})} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 focus:outline-none focus:border-blue-500" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase">Exchange Rate (1 USD = ? {settings.currencySymbol})</label>
                  <input type="number" value={settings.exchangeRate} onChange={e => setSettings({...settings, exchangeRate: parseFloat(e.target.value)})} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 focus:outline-none focus:border-blue-500" />
                </div>
                <div className="space-y-2">
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
              <div className="flex items-center justify-between">
                ...
                {Object.keys(pendingAdminChanges).length > 0 && (
                  <button 
                    onClick={handleSaveAdmins}
                    disabled={saving}
                    className="px-6 py-2 bg-emerald-600 rounded-xl font-bold flex items-center gap-2 hover:bg-emerald-700 transition-all"
                  >
                    {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                    Save Changes
                  </button>
              ...
              <div className="flex gap-4">
                <input 
                  type="text" 
                  placeholder="Enter User Telegram ID..."
                  value={adminIdInput}
                ...
                <button 
                  ...
                  className="px-6 py-3 bg-blue-600 rounded-xl font-bold hover:bg-blue-700 transition-all flex items-center gap-2"
                >
                  <Plus className="w-5 h-5" /> Add to List
                </button>
              </div>
              <p className="text-xs text-slate-500">Note: Changes will not be permanent until you click "Save Changes".</p>
            </div>

            <div className="space-y-4">
              <h3 className="text-lg font-bold px-2">Admin List</h3>
              {allUsers
                .map(u => {
                  const isPendingAdmin = pendingAdminChanges[u.telegramId];
                  const currentIsAdmin = isPendingAdmin !== undefined ? isPendingAdmin : u.isAdmin;
                  return { ...u, currentIsAdmin };
                })
                .filter(u => u.currentIsAdmin)
                .map(u => (
                <div key={u.telegramId} className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <img src={u.photoUrl} className="w-12 h-12 rounded-xl object-cover border border-slate-800" referrerPolicy="no-referrer" />
                    <div>
                      <p className="font-bold">
                        {u.firstName} 
                        {u.telegramId === ADMIN_TG_ID && <span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full ml-2">OWNER</span>}
                        {pendingAdminChanges[u.telegramId] === true && <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full ml-2">PENDING ADD</span>}
                      </p>
                      <p className="text-xs text-slate-500">@{u.username} • ID: {u.telegramId}</p>
                    </div>
                  </div>
                  {u.telegramId !== ADMIN_TG_ID && (
                    <button 
                      onClick={() => handleRemoveAdmin(u.telegramId)}
