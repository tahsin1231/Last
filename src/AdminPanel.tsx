...
━━━━━━━━━━━━━━━━━━━━━`,
  ...
  const handleRemoveAdmin = async (telegramId: string) => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        setAllUsers(prev => prev.map(u => u.telegramId === telegramId ? { ...u, isAdmin: false } : u));
      }
  ...
  const handleUpdateBalance = async (telegramId: string) => {
    ...
    try {
      ...
      setAllUsers(prev => prev.map(u => u.telegramId === telegramId ? { ...u, earnings: amount } : u));
      setEditingUser(null);
      setNewBalance('');
            ...
            <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input value={settings.appLogo} onChange={e => setSettings({...settings, appLogo: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase">Daily Ad Limit (0 for unlimited)</label>
                  <input type="number" value={settings.dailyAdLimit} onChange={e => setSettings({...settings, dailyAdLimit: parseInt(e.target.value)})} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3" />
                </div>
              </div>
