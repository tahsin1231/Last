    ...
    const unsubscribeUser = onSnapshot(doc(db, 'users', userDocId), async (docSnap) => {
      try {
        if (docSnap.exists()) {
          ...
          if (userDocId === ADMIN_TG_ID && !data.isAdmin) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em]">Loading Settings...</p>
        </div>
      </div>
    );
  }
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
          {currentSettings.appLogo && (
            <div className="relative w-32 h-32 rounded-[2.5rem] overflow-hidden border-2 border-slate-800 shadow-2xl mx-auto bg-slate-900 flex items-center justify-center">
              <img src={currentSettings.appLogo} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </div>
          )}
          ...
          {error || loadTimeout ? (
            ...
            <>
              {currentSettings.appName && <h1 className="text-3xl font-black tracking-tight text-white">{currentSettings.appName}</h1>}
              <div className="flex flex-col items-center gap-4">
