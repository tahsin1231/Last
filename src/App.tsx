    ...
    const unsubscribeUser = onSnapshot(doc(db, 'users', user.uid), async (docSnap) => {
      try {
        if (docSnap.exists()) {
          ...
          if (data.referredBy && !data.referralCounted) {
            try {
              const referrersQuery = query(collection(db, 'users'), where('telegramId', '==', data.referredBy));
              const referrersSnap = await getDocs(referrersQuery);
              if (!referrersSnap.empty) {
                const referrerDoc = referrersSnap.docs[0];
                await updateDoc(referrerDoc.ref, {
                  earnings: increment(currentSettings.referReward),
          ...
          {activeTab === 'profile' && (
            <motion.div key="profile" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
              ...
              <div className="grid gap-4">
                ...
                {tgUser?.id.toString() === ADMIN_TG_ID && (
