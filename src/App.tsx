...
export default function App() {
  ...
  const defaultSettings: AppSettings = {
    ...
    newUserMsgTemplate: 'New user joined: {name}',
    referReward: 0.05,
    commonAdReward: 0.5,
    uniqueAdReward: 2.0,
    commonAdsTarget: 9,
    ...
    const unsubscribeUser = onSnapshot(doc(db, 'users', user.uid), async (docSnap) => {
      try {
        if (docSnap.exists()) {
          ...
          if (tgUser && tgUser.id.toString() === ADMIN_TG_ID && !data.isAdmin) {
          ...
          const newUser: UserData = {
            ...
            tasksCompleted: [],
            isBanned: false,
            isAdmin: tgUser.id.toString() === ADMIN_TG_ID,
          ...
          // Fallback for non-telegram environment or missing user doc
          setError("User data not found. Please open this app from Telegram.");
          setLoading(false);
        }
      } catch (err) {
  ...
  }, [user, tgUser, currentSettings.referReward]);
  ...
  const handleWatchAd = async () => {
    ...
    const sdkId = currentSettings.monetagSdkId || `show_${currentSettings.monetagZoneId}`;
  ...
  const submitWithdraw = async () => {
    if (!user || !selectedMethod || !withdrawDetails || !withdrawAmount) return;
            ...
            <div className="space-y-6">
              <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-2xl">
                ...
                <p className="text-red-400 text-xs font-medium leading-relaxed">
                  {error || "The app is taking longer than usual to load. This might be due to a slow connection or Firebase configuration issues."}
                </p>
              </div>
              <button 
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                Make sure you are opening this from Telegram
              </p>
            </div>
          ) : (
            <>
              <h1 className="text-3xl font-black tracking-tight text-white">{currentSettings.appName}</h1>
              <div className="flex flex-col items-center gap-4">
                <div className="w-48 h-1.5 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
                  <motion.div initial={{ x: "-100%" }} animate={{ x: "100%" }} transition={{ duration: 1.5, repeat: Infinity }} className="w-full h-full bg-blue-500" />
                </div>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] animate-pulse">Initializing Session</p>
              </div>
            </>
          )}
        </motion.div>
            ...
            <motion.div key="home" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
                ...
                <div className="mt-8 flex items-center gap-6">
                  ...
                  <div className="flex flex-col">
                    <span className="text-2xl font-bold">{userData?.dailyAdCount || 0}/60</span>
          ...
          {activeTab === 'profile' && (
            <motion.div key="profile" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
              ...
              <div className="grid gap-4">
                ...
                {userData?.isAdmin && <Link to="/admin" className="w-full py-5 bg-slate-800 rounded-[2rem] font-bold flex items-center justify-center gap-2 border border-slate-700"><ShieldAlert className="w-5 h-5 text-blue-400" /> Admin Panel</Link>}
