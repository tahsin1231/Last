    ...
    const unsubscribeUser = onSnapshot(doc(db, 'users', userDocId), async (docSnap) => {
      try {
        if (docSnap.exists()) {
          ...
          if (ADMIN_TG_IDS.includes(userDocId) && !data.isAdmin) {
            ...
            <motion.div key="home" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem]">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Ad Earning</p>
                  <p className="text-xl font-bold">{currentSettings.currencySymbol}{(currentSettings.commonAdReward / (currentSettings.commonAdsTarget + 1)).toFixed(4)}</p>
                </div>
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem]">
